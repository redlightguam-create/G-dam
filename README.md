...
3. Keep the two server windows open:
...
The batch launcher bypasses PowerShell execution-policy issues and starts both the backend and frontend for you.

## Real-Use Deployment

This is the production-style setup. Every user signs in with their own Google account, and G-DAM uses that user's Drive, Docs, and Gmail permissions.

The app now supports:

- Google login from the hosted website
- Per-user sessions
- Encrypted per-user Google credentials
- User-specific `My Drive / G-DAM` folders
- Public split sheet signature links mapped back to the correct user account

Use these services to keep launch work low:

- Vercel for the frontend
- Render for the FastAPI backend
- Supabase Postgres for the auth/session/token database
- Google Cloud OAuth for Google login
...
git status
git add .
git commit -m "Prepare real user Google login"
git push
```

If `git push` says no remote is set, create a GitHub repo first and connect this project to it.

### 2. Create A Supabase Database

Create a Supabase project and copy the Postgres connection string. Use the direct/session connection string as `DATABASE_URL`.
...
7. Add this local authorized redirect URI now:

```text
http://127.0.0.1:8000/auth/google/callback
```

8. After Render gives you the backend URL, come back and add:

```text
https://YOUR-RENDER-BACKEND.onrender.com/auth/google/callback
```

9. Download the web client JSON as `client_secrets.json`.
10. Put it here:

```text
C:\Users\Head Huncho Guam\Desktop\MUSIC DISTRO PROGRAM\client_secrets.json
```

### 4. Convert Google Credentials To Base64
...
Copy the output. That is your `GOOGLE_CLIENT_SECRETS_BASE64`.
...
REQUIRE_LOGIN=1
GDAM_HOSTED=1
PUBLIC_APP_URL=https://YOUR-RENDER-BACKEND.onrender.com
FRONTEND_URL=https://YOUR-VERCEL-APP.vercel.app
CORS_ORIGINS=https://YOUR-VERCEL-APP.vercel.app,http://127.0.0.1:5173,http://localhost:5173
DATABASE_URL=YOUR-SUPABASE-POSTGRES-URL
TOKEN_ENCRYPTION_SECRET=ANY-LONG-RANDOM-SECRET
GOOGLE_CLIENT_SECRETS_BASE64=BASE64-OF-WEB-client_secrets.json
SESSION_COOKIE_SECURE=1
SESSION_COOKIE_SAMESITE=none
```

To create `GOOGLE_CLIENT_SECRETS_BASE64` from PowerShell:
...
After the first deployment, copy the Vercel URL into Render's `FRONTEND_URL` and `CORS_ORIGINS`.

### 7. Update Google And Render URLs

After Vercel and Render both have real URLs:

1. Add this Google authorized redirect URI:
...
3. Google asks for Drive, Docs, and Gmail permission.
4. G-DAM creates or reuses `My Drive / G-DAM` in that user's Drive.
5. All artist folders, song folders, credits, split sheets, and signatures belong to that user's Google account.
...
If a port is already in use, close the old backend/frontend PowerShell window and run `.\Start-Web-Dashboard.bat` again.
