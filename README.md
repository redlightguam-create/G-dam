...
Phase 1 makes the web dashboard public while still using one manager Google Drive account behind the scenes.

- Frontend: Vercel
- Backend API: Render
- Google Drive/Gmail access: the manager token generated by your local login

This is good for a manager-controlled demo. It is not the final multi-user version where every visitor signs into their own Google account.

### 1. Prepare Hosted Secrets

Run this locally after you have already signed into Google through the app:

```powershell
cd "C:\Users\Head Huncho Guam\Desktop\MUSIC DISTRO PROGRAM"
.\Export-Hosted-Secrets.bat
```
...
Do not commit or share that file. It contains access to the manager Google account permissions granted to this app.
...
4. Add these environment variables from `hosted-secrets.txt`:
...
After Render deploys, your backend URL will look like:

```text
https://g-dam-api.onrender.com
```
...
When Vercel gives you the site URL, go back to Render and update `CORS_ORIGINS` so it includes the real Vercel URL.

### 4. Test The Public App

Open the Vercel URL. The Songs, Artists, Collaborators, uploads, credits, and split sheet actions should call the Render backend, and Render should use the manager Google Drive token.

If the frontend loads but data does not, check:

- Vercel `VITE_API_BASE_URL` points to the Render backend.
- Render `CORS_ORIGINS` includes the exact Vercel URL.
- Render has both Google secret variables from `hosted-secrets.txt`.
- The local manager token was created before running `Export-Hosted-Secrets.ps1`.

## Phase 2 Real User Login

Phase 2 is the real-use setup. Every user signs in with their own Google account, and G-DAM uses that user's Drive, Docs, and Gmail permissions.
...
7. Add this authorized redirect URI after Render gives you the backend URL:

```text
https://YOUR-RENDER-BACKEND.onrender.com/auth/google/callback
```

8. Download the web client JSON as `client_secrets.json`.
...
Do not use `GOOGLE_DRIVE_TOKEN_BASE64` for Phase 2. Users create their own Google token by signing in.
